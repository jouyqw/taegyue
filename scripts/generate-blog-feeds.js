...
const posts = blogFiles.map((file) => {
  ...
  const date = match(html, /"datePublished"\s*:\s*"([^"]+)"/i)
    ...
    || today;
  return { file, slug, url, title, description, date };
}).sort((a, b) => b.date.localeCompare(a.date) || a.file.localeCompare(b.file));

const blogCards = posts.length
  ? posts.map((post) => `        <a class="card" href="/blog/${post.slug}">
          <span>${escapeXml(post.date)}</span>
...
const sitemapUrls = [
  ...
  ...posts.map((post) => ({ loc: post.url, changefreq: 'monthly', priority: '0.9', lastmod: post.date }))
];
...
const rss = [
  ...
  '<rss version="2.0">',
  '  <channel>',
  '    <title>법무법인 태앤규 전주 개인회생 칼럼</title>',
  `    <link>${site}/blog/</link>`,
  '    <description>전주 개인회생, 개인파산, 채무조정 법률칼럼</description>',
